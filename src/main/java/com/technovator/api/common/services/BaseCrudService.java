...
import com.technovator.api.common.annotations.AuditField;
...
import com.technovator.api.common.annotations.AuditField.AuditFieldType;
import com.technovator.api.common.auditlog.AuditLog;
import com.technovator.api.common.auditlog.AuditLogService;
import com.technovator.api.common.auditlog.Auditable;
...
import com.technovator.api.common.events.ResourceChangeEvent;
...
import com.technovator.api.common.utils.BeanUtil;
import com.technovator.api.common.utils.CommonUtil;
import com.technovator.api.common.utils.DateUtils;
import com.technovator.api.common.utils.EntityUtils;
...
public abstract class BaseCrudService<T, ID extends Serializable> extends BaseEntityService<T, ID> {
	...
	public List<AuditLog> getHistory(ID id) {
		...
		if (obj != null) {
			return auditService.find((AuditableMain) obj, id, additionalHistory(id));
		} else {
	...
	protected void saveHistory(String action, Auditable obj, String details, String filter) {
		...
		if (obj instanceof AuditableMain) {
			auditService.add(action, (AuditableMain) obj, id, details, filter);
	...
	protected void saveChildHistory(String action, AuditableChild<?> obj, String details) {
		...
		details += " [" + obj.getName() + "]";
		auditService.add(Actions.Crud.Update, (AuditableMain) parent, parentId, details, obj.getClass().getSimpleName());
	}
	...
	protected void saveReferenceHistory(String action, AuditableReference<?, ?> obj, String details) {
		...
		AuditableMain ref = EntityManagerProvider.getEntityManager().find(obj.getReferenceEntity(),
				obj.getReferenceId());
		if (ref == null) {
